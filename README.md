...
### 콜센터 STT 품질 검증 프로젝트 (후처리 포함 최저 CER 0.0761, 2025.03 - 2025.05)
- **설명**: 원본 음성 1,000건을 직접 청취·검수하고, 무음 8건을 제외한 992개 유효 샘플을 대상으로 30초를 넘는 파일을 중간 묵음 지점에서 수동 분할했습니다. 묵음 구간에서 발생하는 특수문자와 앞 문장 반복 문제를 줄인 뒤 Whisper 모델 비교·파인튜닝·후처리 실험을 수행했습니다.
- **입력 처리**: PCM/WAV를 16kHz·mono·16-bit WAV로 변환했습니다. 코덱 종류는 2~3개 수준이었으며 코덱별 별도 평가지표는 만들지 않았습니다.
- **핵심 결과**: 평가 가능한 992개 음성 샘플에서 Whisper 기본 모델 6종을 CER 기반으로 비교해 Large v3 Turbo 평균 CER 0.0889를 확인했습니다.
- **고오류 분석**: 992개 평가 샘플 중 CER 0.3 이상인 인식 품질이 낮은 22개를 선별해 학습된 여러 모델로 재추론하고 결과를 비교했습니다.
...
- **노이즈 전처리 실험**: 400~3200Hz 밴드패스 필터 코드를 작성했으나 CER 개선 효과는 검증하지 않아 최종 성과에 포함하지 않았습니다.
- **기술**: Python, PyTorch, Hugging Face Transformers, Whisper, CER, LoRA/PEFT, Google Colab
- **추후 과제**: 자동 묵음 구간 분할, 화자 분리 기반 CER 개선 효과 검증, 밴드패스 필터 효과 검증, 운영 환경 연동 및 실제 배포
...
- **기술**: KcBERT, KF-DeBERTa, GPT2LMHeadModel, RAG, STT&TTS, Entity extraction
...
- **성과**: 최종 KcBERT 분류 모델에서 Accuracy 0.96 · F1 0.96을 기록했고, BLEU 스코어 73점을 달성했습니다.
- **기술**: KcBERT, KcELECTRA, RNN, LSTM
- [[프로젝트 리포지토리](https://github.com/LittlePrince327/X_filter.git)]

### 전통주 추천 알고리즘 프로젝트 (2023.08)
- **설명**: 사용자 취향에 맞는 전통주를 추천하는 시스템을 구축했습니다.
- **성과**: KNN 분류 실험에서 정확도 85%를 기록했습니다.
